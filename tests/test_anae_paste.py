...
LU1073896877     27/05/2026
    113.50772     -     13,45 €     1 318,40 €     10,59 €     0,70 %'''
...
class TestParsing:
    ...
    def test_pru_derived_from_latentes(self):
        # cost_basis (total) = Montant - (+/- latentes) = 5412.37 - 682.87
        l = _by_isin(parse_anae_paste(SAMPLE))['FR0013297546']
        assert l.cost_basis == 4832.06
        # gain% coherent avec Anae (10,40 %)
...
class TestSeparatorRobustness:
    ...
    def _check(self, text):
        ...
        d = _by_isin(r.lines)
        assert d['FR0013297546'].market_value == 5412.37
        assert d['FR0013297546'].cost_basis == 4832.06
    ...
    def test_nbsp_thousands(self):
        ...
        nb = chr(0x00a0)
        t = SAMPLE.replace('5 566', '6' + nb + '398').replace('1 526', '1' + nb + '526')
        self._check(t)
...
class TestFullImport:
    ...
    def test_auto_split_by_asset_class(self, client):
        ...
        assert by_cat['Fond Euro']['value'] == 41205.16
        assert round(by_cat['Actions']['value'], 2) == 5412.37
        assert round(by_cat['Obligations']['value'], 2) == 2604.48
