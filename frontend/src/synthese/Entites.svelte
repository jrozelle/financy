        ...
        {#each lignes as l (l.e.name)}
          <tr>
            ...
            <td style="text-align:right">
              {fmt(l.netFamille)}
              <span style="font-size:11px;color:var(--text-muted);margin-left:4px">{l.pctFamille}</span>
            </td>
            {#if !famille}
              <td style="text-align:right;font-weight:700;color:var(--primary)">
                {fmt(l.netTitulaire)}
                {#if l.pctTitulaire !== null}<span style="font-size:11px;color:var(--text-muted);margin-left:4px">{fmtPct(l.pctTitulaire * 100, 0)}{
